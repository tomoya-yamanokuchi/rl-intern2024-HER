...
class PendulumDomainInfo(object):
    ...
    def __init__(self, userDefinedSettings, domain_range=None):
        ...
        self.set_domain_parameter_all_space(
            domain_range=domain_range, randomization_flag=userDefinedSettings.DOMAIN_RANDOMIZATION_FLAG)
    ...
    def set_domain_parameter_all_space(self, domain_range=None, randomization_flag=True):
        if randomization_flag:
            ...
            sampling_method = 'fix'
        self.dt = DomainParameter(name='dt', initial_value=0.05, min_value=0.045,
                                  max_value=0.055, sampling_method=sampling_method)
        self.g = DomainParameter(name='g', initial_value=10., min_value=7,
                                 max_value=15., sampling_method=sampling_method)
        self.m = DomainParameter(name='m', initial_value=1., min_value=0.8,
                                 max_value=1.2, sampling_method=sampling_method)
        self.l = DomainParameter(name='l', initial_value=1., min_value=0.8,
                                 max_value=1.2, sampling_method=sampling_method)
        self.torque_weight = DomainParameter(
            name='torque_weight', initial_value=1., min_value=0.8, max_value=1.5, sampling_method=sampling_method)
        self.torque_bias = DomainParameter(
            name='torque_bias', initial_value=0, min_value=-0.5, max_value=0.5, sampling_method=sampling_method)
    ...
    def get_domain_parameters(self, normalize_flag=False):
        ...
        for parameter in [self.dt, self.g, self.m, self.l, self.torque_weight, self.torque_bias]:
            if normalize_flag:
                value = (parameter.value - parameter.min_value) / \
                    (parameter.max_value - parameter.min_value)  # (a,b)->(0,1)
    ...
    def normalize(self, domain_parameters):
        ...
        normalized_domain_parameters = (
            domain_parameters - min_value) / (max_value - min_value)  # (a,b)->(0,1)
...
class DomainParameter(object):
    def __init__(self, name, initial_value, min_value, max_value, sampling_method):
        assert (initial_value >= min_value) and (initial_value <=
                                                 max_value), 'domain initial value is out of range'
        self.name = name
    ...
    def set(self, set_value=None, set_range=None, set_method=None):
        if set_method == 'direct_set':
            ...
            self.value = (self.max_value - self.min_value) * \
                set_value + self.min_value
        else:
            if self.sampling_method == 'uniform':
                if set_range is not None:
                    self.set_divided_space(set_range)
                origin_sample = np.random.rand()
                shifted_sample = (self.max_range - self.min_range) * \
                    origin_sample + self.min_range
                self.value = (self.max_value - self.min_value) * \
                    shifted_sample + self.min_value
        ...
        assert (self.value >= self.min_value) and (self.value <= self.max_value), 'domain value is out of range: {} < {} < {}'.format(
