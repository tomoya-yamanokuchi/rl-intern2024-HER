...
class ModelDomainObjectBuilder:
    def __init__(self, config):
        self.domain_object = ModelDomainObject()
    ...
    def build_content_encoder(self):
        ...
        content_encoder = ContentEncoderFactory.create(
            dim_frame_feature = self.domain_object.config.model.frame_encoder.dim_frame_feature,
            # dim_lstm_hidden   = self.domain_object.config.model.forward_recurrent_encoder.dim_lstm_hidden,
    ...
    def build_motion_encoder(self):
        ...
        motion_encoder = MotionEncoderFactory().create(
            dim_a         = self.domain_object.config.model.frame_encoder.dim_frame_feature,
    ...
    def build_content_prior(self):
        from cdsvae.domain.model.generative_model.content_prior.ContentPrior import ContentPrior
        content_prior = ContentPrior(self.dim_content)
    ...
    def build_frame_decoder(self):
        ...
        frame_decoder = FrameDecoderFactory().create(
            **self.domain_object.config.model.frame_decoder,
            ...
            out_channels        = self.domain_object.config.model.frame_encoder.in_channels,
        )
        import ipdb; ipdb.set_trace()
    ...
    def get_domain_object(self) -> ModelDomainObject:
